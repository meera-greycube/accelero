# Copyright (c) 2026, GC and contributors
...
def get_data_with_qty_to_make(filters):
	...
	lead_times = batch_fetch_lead_times(bom_data)
	qty_to_make = flt(filters.get("qty_to_make"))
	safety_cushion = flt(filters.get("safety_cushion")) / 100
	...
	for row in bom_data:
		...
		lead_time = lead_times.get(row.item_code, 0)
		minimum_level = flt(lead_time) * flt(required_qty)
		reorder_qty = minimum_level * (1 + safety_cushion)
		...
		data.append(
			{
				...
				"lead_time": lead_time,
				"minimum_level": fmt_qty(minimum_level),
				"reorder_qty": fmt_qty(reorder_qty),
			}
		)
...
def batch_fetch_lead_times(bom_data):
	if not bom_data:
		return {}
	...
	return {
		r.name: r.custom_lead_time_in_days
		for r in frappe.get_all(
			"Item",
			filters={"name": ["in", item_codes]},
			fields=["name", "custom_lead_time_in_days"],
		)
	}
